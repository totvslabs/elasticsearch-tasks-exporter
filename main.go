...
var (
	version       = "dev"
	app           = kingpin.New("elasticsearch-tasks-exporter", "exports elasticsearch pending tasks metrics in prometheus format")
...
func main() {
	...
	log.Infof("starting elasticsearch-tasks-exporter %s on %s...\n", version, *esURL)

	log.Debug("debug enabled")

	client := client.New(*esURL)
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w,
			`
			<html>
			<head><title>ElasticSearch Tasks Exporter</title></head>
			<body>
				<h1>ElasticSearch Tasks Exporter</h1>
